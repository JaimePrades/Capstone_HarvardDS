...
# analysis purposes, in the modeling section we will use the split data ("edx" and "final_holdout_test")
...
# NOTE: I will use the final_holdout_test set to make the plot because it is possible that on some 
...
# If I fuse the information of genres and rating, I will be able to reach to a nice plot
...
# From this point further, I will use the "edx" dataset to create the model and the "final_holdout_test"
...
movies_contribution<- edx %>% dplyr::group_by(movieId) %>% 
  dplyr::summarize(Mg_contribution_of_movies = mean(rating - ratings_mean))

model_2_prediction <- ratings_mean + final_holdout_test %>% left_join(movies_contribution, by='movieId') %>%
...
user_contribution <- edx %>% left_join(movies_contribution, by='movieId') %>% dplyr::group_by(userId) %>% 
  dplyr::summarize(Mg_contribution_of_users = mean(rating - ratings_mean - Mg_contribution_of_movies))

model_3_prediction <- final_holdout_test %>% left_join(movies_contribution, by='movieId') %>%
...
model_4_prediction <- final_holdout_test %>% left_join(movies_contribution, by = "movieId") %>%
  left_join(user_contribution, by = "userId") %>%
...
# The best model is the one that includes the contribution of the rating's mean, 
# and the combined effect of: movies, users and genres. 
